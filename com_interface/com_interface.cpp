...
int setupNetwork(const std::string &interface)
{
  ...
  if (!found_interface)
  {
    printf("Unable to find IPv4 address of interface %s. Perhaps it needs to be set to a static address?\n", interface.c_str());
    return -1;
  }
  ...
  if(result < 0)
  {
      printf("couldn't set local interface for udp tx sock, fails with errno: %d\n", errno);
  }
  result = setsockopt(tx_sock_, IPPROTO_IP, IP_MULTICAST_LOOP, &loopback, sizeof(loopback));
  if(result < 0)
  {
      printf("couldn't turn off outgoing multicast loopback, fails with errno: %d\n", errno);
  }
  ...
  if(result < 0)
  {
    printf("couldn't set SO_REUSEADDR on UDP RX socket, fails with errno: %d\n", errno);
  } 
  ...
  if(result < 0)
  {
    printf("couldn't bind rx socket to port %d, fails with errno: %d\n", PORT_BASE, errno);
  } 
  ...
  if(result < 0)
  {
    printf("couldn't add to multicast group, fails with errno: %d\n", errno);
  }                
  printf("connected!\n\n");
}