/////////////////////////////////////////////////////////////////////////////
//
//   Copyright 2014 Open Source Robotics Foundation, Inc.
//
//   Licensed under the Apache License, Version 2.0 (the "License");
//   you may not use this file except in compliance with the License.
//   You may obtain a copy of the License at
//
//       http://www.apache.org/licenses/LICENSE-2.0
//
//   Unless required by applicable law or agreed to in writing, software
//   distributed under the License is distributed on an "AS IS" BASIS,
//   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
//   See the License for the specific language governing permissions and
//   limitations under the License.
//
/////////////////////////////////////////////////////////////////////////////

#ifndef REFLEXHAND_H
#define REFLEXHAND_H

#include <netinet/in.h>
#include <boost/function.hpp>
#include <string>

#include <sys/types.h>
#include <sys/socket.h>
#include <arpa/inet.h>
#include <unistd.h>
#include <ifaddrs.h>
#include <netdb.h>
#include <stdio.h>

#include <string>

#include <errno.h>


#define NUM_FINGERS                   3
#define NUM_SENSORS_PER_FINGER        9
#define NUM_SENSORS_PALM              11
#define NUM_TAKKITLE                  (NUM_FINGERS * NUM_SENSORS_PER_FINGER + NUM_SENSORS_PALM)
#define NUM_ENCODERS                  NUM_FINGERS
#define NUM_SERVOS                    4
#define NUM_DYNAMIXELS                4

// FOR PRINTING
#define DYNAMIXEL_ANGLES              0
#define ENCODERS_POSITION             1
#define TAKKTILE_PRESSURES            2
#define RECEIVED_MESSAGE_CHAR         3
#define RECEIVED_MESSAGE_HEX          4

namespace reflex_hand
{
    typedef struct
    {
        uint8_t  header[4];
        uint32_t systime;
        uint16_t tactile_pressures[NUM_TAKKITLE];
        uint16_t tactile_temperatures[NUM_TAKKITLE];
        uint16_t encoders[NUM_FINGERS];
        uint8_t  dynamixel_error_states[4];
        uint16_t dynamixel_angles[4];
        uint16_t dynamixel_speeds[4];
        uint16_t dynamixel_loads[4];
        uint8_t  dynamixel_voltages[4];
        uint8_t  dynamixel_temperatures[4];
    } __attribute__((packed)) mcu_state_format_1_t;

  class ReflexHandState
  {
  public:
    uint32_t systime_us_;
    uint16_t tactile_pressures_[NUM_TAKKITLE];
    uint16_t tactile_temperatures_[NUM_TAKKITLE];
    uint16_t encoders_[NUM_FINGERS];
    uint8_t  dynamixel_error_states_[4];
    uint16_t dynamixel_angles_[4];
    uint16_t dynamixel_speeds_[4];
    uint16_t dynamixel_loads_[4];
    uint8_t  dynamixel_voltages_[4];
    uint8_t  dynamixel_temperatures_[4];
    ReflexHandState();
    ~ReflexHandState();
  };

  class ReflexHand
  {
  public:
    const static int PORT_BASE = 11333;
    static const uint16_t DYN_MIN_RAW = 0;
    static const uint16_t DYN_MAX_RAW = 4095;
    static const uint16_t DYN_MIN_RAW_WRAPPED = 16383;  // For checking negative wraps
    static const float DYN_POS_SCALE = (4 * 2 * 3.141596) / 4095;  // Assuming resolution divider of 4
    static const float DYN_VEL_SCALE = 0.01194;  // rad/s for every velocity command -- http://support.robotis.com/en/product/dynamixel/mx_series/mx-28.htm#Actuator_Address_20
    static const float ENC_SCALE = (2 * 3.141596) / 16383;
    
    enum ControlMode{CM_IDLE = 0, CM_VELOCITY = 1, CM_POSITION = 2};

    typedef boost::function<void(const ReflexHandState * const)> StateCallback;
    void setStateCallback(StateCallback callback);                               // NON ROS

    int setupNetwork(const std::string &interface);                              // NON ROS
    ReflexHand(const std::string &interface);                                    // NON ROS
    ~ReflexHand();                                                               // NON ROS   
    bool listen(const double max_seconds);                                       // NON ROS 
    void setServoTargets(const uint16_t *targets);                               // NON ROS 
    void setServoControlModes(const ControlMode *modes);                         // NON ROS 
    void setServoControlModes(const ControlMode mode);                           // NON ROS 
    // bool happy() { return happy_; }
    void printInfo(uint8_t type, uint8_t *rxbuf = NULL);                         // NON ROS
    ReflexHandState rx_state_;
  private:
    enum CommandPacket { CP_SET_SERVO_MODE = 1, CP_SET_SERVO_TARGET = 2 };
    int tx_sock_, rx_sock_;
    sockaddr_in mcast_addr_;
    StateCallback state_cb_;

    bool happy_;
    void tx(const uint8_t *msg, const uint16_t msg_len, const uint16_t port);    // NON ROS
    void rx(const uint8_t *msg, const uint16_t msg_len);                         // NON ROS
  };

}d

#endif
