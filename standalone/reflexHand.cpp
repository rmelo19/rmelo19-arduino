...
void ReflexHand::tx(const uint8_t *msg, const uint16_t msg_len, const uint16_t port)
{
  ...
  if (nsent < 0)
  {
    printf("woah. sendto() returned %d", nsent);
  }
}
...
void ReflexHand::printInfo(uint8_t type, uint8_t *rxbuf)
{
  switch (type)
  {
    case DYNAMIXEL_ANGLES:
      printf("Dynamixel Angles: ");
    ...
    case ENCODERS_POSITION:
      ...
      break;
    case TAKKTILE_PRESSURES:
      printf("Takktile Pressures:\n");
      for (int j = 0; j < NUM_FINGERS; j++)
      {
        printf("Finger %d: ", j + 1);
        for (int i = NUM_FINGERS*j; i < NUM_FINGERS*(j + 1); i++)
        {
      ...
      break;
    case RECEIVED_MESSAGE_CHAR:
      if(rxbuf == NULL)
      {
        printf("Pointer to buffer not provided.\n");
        break;
      }
      for (int i = 0; i < sizeof(mcu_state_format_1_t); ++i)
      {
        printf("%x ", rxbuf[i]);
      }
      break;
    case RECEIVED_MESSAGE_HEX:
      if(rxbuf == NULL)
      {
        printf("Pointer to buffer not provided.\n");
        break;
